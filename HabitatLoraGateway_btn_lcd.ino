...
#define NSS_PIN D8    //#define NSS_PIN 15
#define DIO0_PIN D1   //#define DIO0_PIN 5

// To force an AP Reset set this pin to low:
#define AP_RST_PIN D0 //#define AP_RST_PIN 16
...
void setup() {
  ...
  display.clear();
  display.drawString(0, 18," LORA GW ");
  display.display();
  ...
  display.clear();
  display.display();
  display.setFont(Open_Sans_Hebrew_Condensed_Light_20);
  ...
  String CONF = "1D:" + Reg_1d + " 1E:" + Reg_1e + " 26:"+ Reg_26;
  display.setFont(Open_Sans_Hebrew_Condensed_Light_22);
  display.drawString(0, 22,CONF);
  display.display();
}
...
void initWifiManager() {
  pinMode(AP_RST_PIN, INPUT);           // set pin to input
  digitalWrite(AP_RST_PIN, HIGH);       // turn on pullup resistors
  ...
  else if( ! wifiManager.autoConnect(gatewayName.c_str())) {
    Serial.println("Timeout connecting. Restarting...");
    delay(1000);
    ESP.reset();
  } 
